...
export class SchoolApi extends MainAPIClient {
  ...
  public createComment(request: SchoolComment): Promise<PutResponse | null> {
    /**
     * コメントを作成
     */
    const url = '/school/create/comment';
    const path = this.combineUrl(url);
    return this.httpPost<SchoolComment, PutResponse>(path, request);
  }
  ...
  public editComment(request: SchoolComment): Promise<PutResponse | null> {
    /**
     * コメントを編集
     */
